...
class Driver:
    def drive(self, image):
        ...
        '''
        이 위를 보면 상수들이 함수 안에 정의돼있다. Python은 스크립트 언어이므로, 이렇게 정의하면 매 함수가 호출될 때마다 상수가 재정의되어 속도가 조금 느려진다.
        물론 상수 몇 개를 정의하는 것은 영상처리에 비해 비교할 수 없을 만큼 빠르므로, 전체 실행 속도에 큰 영향을 주지는 않을 것이다.
        그러나 클래스 사용법을 익히기 위해, Python에 익숙한 사람이라면 init 함수를 사용하여, 상수를 한 번만 정의하고 이후에는 계속 사용할 수 있도록 만들어보자.
        
        특히 조금 더 우수한 주행 알고리즘을 만들기 위해 이전 프레임의 결과를 일부 기억해뒀다가 사용하는 것도 가능하다.
        그렇게 하려면 반드시 self를 사용하여, 지역변수가 아닌 객체 자체의 변수에 값을 저장해두어야 한다.
        '''
