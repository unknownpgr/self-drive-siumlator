...
class Driver:
    def drive(self, image):
        ...
        # 차선의 중심 위치를 dist라고 가정한 후, 왼쪽 차선의 위치를 left, 오른쪽 차선의 위치를 right라고 하자.
        # 이때 이미지에서 left위치와 right위치에 실제로 255인 픽셀이 많이 있다면, 그 부분이 차선일 확률이 높은 것이다.
        ...
        # 앞서 검출한 optimal_cetner 값은 0~이미지 가로길이 범위를 가진다.
